 ...
 * and a custom 'verilance-role-change' event for same-tab updates.
 */
export function useRole() {
    ...
    useEffect(() => {
        ...
        const handleStorage = (e) => {
            if (e.key === 'verilance-role' && e.newValue) {
                setRole(e.newValue);
            }
        };
        ...
        const handleCustom = () => {
            setRole(localStorage.getItem('verilance-role') || 'client');
        };

        window.addEventListener('storage', handleStorage);
        window.addEventListener('verilance-role-change', handleCustom);

        // Poll as a fallback (every 300ms) — cheap since it's just a localStorage read
        const interval = setInterval(() => {
            const current = localStorage.getItem('verilance-role') || 'client';
            setRole((prev) => (prev !== current ? current : prev));
        }, 300);

        return () => {
            window.removeEventListener('storage', handleStorage);
            window.removeEventListener('verilance-role-change', handleCustom);
